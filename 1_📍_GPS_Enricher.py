...
if uploaded_file:
    ...
    MIN_GAP_KM = st.slider(
        ...
        0.0, 10.0, 2.0, 0.5,
        format="%.1f km",
        help="Higher = Cleaner map. Lower = More data.",
...
BATCH_SIZE = 25
WORKERS = 4
HEADERS = {"User-Agent": "GPX-Enricher/24.0", "Referer": "https://streamlit.io/"}
MIRRORS = [
...
if st.session_state.running:
    ...
    try:
        ...
        if not df.empty:
            ...
            if original_name.lower().endswith(".gpx"): stem = original_name[:-4]
            else: stem = original_name
