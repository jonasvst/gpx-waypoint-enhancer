...
with c2:
    st.subheader("📍 GPS Enricher")

# --- USER GUIDE (UPDATED) ---
with st.expander("📘 **User Guide: Logic & Legend**"):
    st.markdown("""
    ### **1. Smart Scanning Strategy**
    This tool is built for speed and efficiency.
    * **Zero Detour:** We scan a **50m radius** along your track. If a fountain is 200m away, we ignore it.
    * **No Blind Spots:** We sample every **100m** to guarantee full coverage of your route.
    * **Smart De-Clutter:** If we find a Gas Station, we hide other Gas Stations for the next **2km** (configurable). This keeps your map readable in towns without hiding other essentials like Water.

    ### **2. Deep Data Extraction**
    We extract critical logistics from OpenStreetMap tags:
    * **Identity:** Brand names ("Shell", "Coop") instead of generic labels.
    * **Logistics:** **Opening Hours** (crucial for night riding) and **Phone Numbers**.
    * **Attributes:** Water drinkability, Toilet fees, and Indoor/Outdoor status.
...
amenity_config = {
    "Water": {
        "label": "💧 Water",
        "desc": "Includes drinking fountains, springs, public taps, and watering places.",
        "query": """(node["amenity"~"drinking_water|fountain|watering_place"](around:{radius},{coords});node["natural"~"spring"](around:{radius},{coords});node["man_made"~"water_tap|water_well"](around:{radius},{coords});)""",
        "icon": "Water", "color": [0, 128, 255]
    },
    "Cemetery": {
        "label": "⚰️ Cemeteries",
        "desc": "Often reliable sources of water. Marked with a Water icon.",
    ...
    "Shops": {"label": "🛒 Shops", "desc": "Supermarkets, convenience stores, kiosks, and bakeries.", "query": """node["shop"~"supermarket|convenience|kiosk|bakery|general"](around:{radius},{coords})""", "icon": "Convenience Store", "color": [0, 200, 0]},
    "Fuel": {"label": "⛽ Fuel", "desc": "Gas stations (often 24/7 food/water).", "query": """node["amenity"~"fuel"](around:{radius},{coords})""", "icon": "Gas Station", "color": [255, 140, 0]},
    "Food": {"label": "🍔 Food", "desc": "Restaurants, fast food, and cafes.", "query": """node["amenity"~"restaurant|fast_food|cafe"](around:{radius},{coords})""", "icon": "Food", "color": [0, 200, 0]},
    "Sleep": {"label": "🛏️ Sleep", "desc": "Hotels, hostels, and guest houses.", "query": """node["tourism"~"hotel|hostel|guest_house"](around:{radius},{coords})""", "icon": "Lodging", "color": [128, 0, 128]},
    "Camping": {"label": "⛺ Camping", "desc": "Official campsites.", "query": """node["tourism"~"camp_site"](around:{radius},{coords})""", "icon": "Campground", "color": [34, 139, 34]},
    "Bike": {"label": "🔧 Bike", "desc": "Bicycle shops for repairs/parts.", "query": """node["shop"~"bicycle"](around:{radius},{coords})""", "icon": "Bike Shop", "color": [255, 0, 0]},
    ...
    "Train": {"label": "🚆 Train", "desc": "Train stations (Bail-out points).", "query": """node["railway"~"station|halt"](around:{radius},{coords})""", "icon": "Generic", "color": [50, 50, 50]}
}
...
if uploaded_file:
    ...
    # Render Checkboxes with Tooltips
